...
# Import excel sheet
raw.data <- read_excel("Measurements timeline.xlsx", sheet = "Overview (monthly)")
...
colnames(frag.data) <- c("Species", "Frags")
...
frag.data <- mutate(frag.data, Index = paste(Abbreviation, Frags, sep = "\n"))

# Genus totals
genus.data <- as.data.frame(tapply(frag.data$Frags, frag.data$Genus, FUN = sum, na.rm = TRUE))
colnames(genus.data) <- c("Frags")
genus.data <- tibble::rownames_to_column(genus.data, "Genus")
genus.data <-genus.data[order(-genus.data$Frags),]
...
treemap(frag.data, index=c("Genus", "Index"), vSize="Frags",  type="index",
...
treemap(frag.data, index=c("Genus", "Species"), vSize="Frags",  type="index",
        fontsize.labels=c(2,2), fontcolor.labels=c("transparent","#292929"), fontface.labels=c(4,3),
        bg.labels=c("transparent"), inflate.labels = T, overlap.labels=1, lowerbound.cex.labels = 0,
        align.labels=list(c("center", "center"), c("center", "center")), 
        border.col=c("#FFFFFF","#F4F4F4"), border.lwds=c(6,2),
...
tiff(file="Treemap_species.tiff", width=23, height = 12, units="cm", res=300)
treemap(frag.data, index=c("Genus", "Abbreviation"), vSize="Frags",  type="index",
        ...
        align.labels=list(c("center", "center"), c("left", "top")), xmod.labels = 0, ymod.labels = -0.01,
        border.col=c("#FFFFFF","#F4F4F4"), border.lwds=c(6,2),
        palette = Pal,
